...
1. **Read AGENTS.md First**: Every agent must read this file and [`docs/AGENT_SOP.md`](docs/AGENT_SOP.md) before making any code or documentation edits.
...
4. **Branch & PR Policy**: Always work on development branches (`dev/...`) and submit Pull Requests. Direct commits to `main` are strictly forbidden.
...
14. **Update STATUS Bilingually**: Track milestone progress and readiness in both `STATUS.md` and `STATUS.zh-CN.md`.
...
17. **Do Not Hallucinate Native ABI**: Adhere strictly to verified native C declarations and documented signatures in `TobiiStreamEngineNative.cs` and `docs/ABI_PROVENANCE.md`.
18. **Respect Architectural Decisions**: Check existing Architecture Decision Records (`docs/adr/`) before designing new components or creating new ADRs.
...
- [Agent Standard Operating Procedure (English)](docs/AGENT_SOP.md)
- [智能体标准操作规范 (简体中文)](docs/AGENT_SOP.zh-CN.md)
