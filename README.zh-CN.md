...
- **工业级重连状态机**：针对 USB 线碰掉拔插、系统休眠唤醒、驱动后台服务重启等真实场景，采用带抖动的指数退避重连机制自动恢复。
- **物理像素级坐标转换**：将传感器归一化凝视坐标精确映射至主显示器物理像素，在 100%、125%、150%、200% Windows 显示缩放下均保持精准击键。
...
3. **软件依赖**：
   ...
   - [OptiKey](https://github.com/OptiKey/OptiKey/releases) 版本 **4.1.0 或更高**（已在 4.1.1、4.2.0、4.2.2 验证）。
...
2. 解压到以下目录：
   ```
   %LocalAppData%\OptiKey\OptiKey\EyeTrackerPlugins\yuanweize\OptiKey-ET5-Plugin\<版本号>\
   ```
...
本项目采用 **GNU General Public License v3.0** (GPL-3.0) 授权协议。详情参见 [LICENSE](LICENSE)。
