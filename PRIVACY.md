...
1. **Ephemeral Real-Time Processing Only**:
   - Gaze coordinates received from the Tobii sensor are processed purely in volatile memory for the sole purpose of immediate on-screen key selection.
...
2. **Zero Local Storage of Gaze Data**:
   ...
   - Log files record ONLY high-level lifecycle events (e.g., `Connected`, `Reconnecting`, `Disconnected`, error codes). Raw $(X, Y)$ points and device URLs are **never logged**.
...
4. **Diagnostic Tool Redaction**:
   ...
   - Hardware serial numbers and URLs are redacted in diagnostic logs.
